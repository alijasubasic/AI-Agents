...
The supervisor reviews decisions the specialist agents have already made. The
...
- Codex article A1 states the same rule at the source: a specialist agent's
  escalation is final, and the supervisor records it rather than reweighing it.
...
- **It makes adding oversight risk-free.** Whatever the supervisor does, the outcome
...
- **A false escalation is permanent.** If a specialist holds something it
  should not have, the supervisor cannot release it. Only a person can, and until
...
- **The autonomy rate is bounded by the strictest component.** In the demo it
  ...
  the supervisor concluded itself. Raising it means improving the specialists, not
  the supervisor.
- **The supervisor cannot fix a bad decision, only stop it.** There is no path where
