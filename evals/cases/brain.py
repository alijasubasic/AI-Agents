...
from agents.brain.models import Decision, DecisionKind, Judgement, Verdict
from agents.brain.reporting import tasks_from_reviews
...
def _report():
    return brain_demo.run(_settings())
...
@case(
    id="brain-supervision-never-loosens",
    agent=AGENT,
    layer=Layer.LOGIC,
...
def _() -> Score:
    ...
    for source, relaxed in itertools.product(sources, [True, False]):
        codex_only = BrainAgent(settings=_settings()).review(source).verdict
        with_model = (
            BrainAgent(provider=_judge(recommend_hold=relaxed), settings=_settings())
            .review(source)
            .verdict
        )
...
@case(
    id="brain-escalation-is-final",
    agent=AGENT,
    layer=Layer.LOGIC,
    description="A specialist's escalation survives a reviewer that sees no problem.",
)
def _() -> Score:
    review = BrainAgent(provider=_judge(recommend_hold=False), settings=_settings()).review(
...
@case(
    id="brain-reviewer-can-still-tighten",
    agent=AGENT,
    layer=Layer.LOGIC,
...
@case(
    id="brain-blocked-decisions-skip-the-model",
    agent=AGENT,
    layer=Layer.LOGIC,
...
def _() -> Score:
    ...
    review = BrainAgent(provider=provider, settings=_settings()).review(
        _decision(recipient_verified=False)
    )
    return combine(
...
@case(
    id="brain-unverified-claim-is-blocked-from-going-out",
    agent=AGENT,
    layer=Layer.LOGIC,
...
@case(
    id="brain-internal-notes-may-hold-unverified-claims",
    agent=AGENT,
    layer=Layer.LOGIC,
...
@case(
    id="brain-unconfirmed-recipient-is-blocked",
    agent=AGENT,
    layer=Layer.LOGIC,
...
@case(
    id="brain-pressure-selling-is-held-not-destroyed",
    agent=AGENT,
    layer=Layer.LOGIC,
...
@case(
    id="brain-chain-blocks-an-unsourced-figure-reaching-a-prospect",
    agent=AGENT,
    layer=Layer.LOGIC,
...
@case(
    id="brain-chain-blocks-mail-to-an-unspoken-address",
    agent=AGENT,
    layer=Layer.LOGIC,
...
@case(
    id="brain-reviewer-catches-what-rules-cannot",
    agent=AGENT,
    layer=Layer.LOGIC,
...
@case(
    id="brain-every-specialist-escalation-survives",
    agent=AGENT,
    layer=Layer.LOGIC,
...
@case(
    id="brain-approved-work-generates-no-task",
    agent=AGENT,
    layer=Layer.LOGIC,
...
@case(
    id="brain-cannot-release-a-false-escalation",
    agent=AGENT,
    layer=Layer.LOGIC,
...
@case(
    id="brain-sees-one-decision-at-a-time",
    agent=AGENT,
    layer=Layer.LOGIC,
...
def _() -> Score:
    brain = BrainAgent(settings=_settings())
...
@case(
    id="brain-honesty-check-misses-paraphrase",
    agent=AGENT,
    layer=Layer.LOGIC,
