...
from agents.supervisor.reporting import build_report, build_sheets, outbound_queue, render_markdown
...
def run(settings: Settings | None = None) -> DailyReport:
    ...
    supervisor = SupervisorAgent(provider=judge_provider(decisions), settings=settings)
...
def _print(report: DailyReport) -> None:
    print(f"\n{'=' * 78}")
    print(f"Decisions reviewed by the supervisor ({len(report.reviews)})")
    print("=" * 78)
...
def main() -> None:
    ...
    print("supervisor demo - every agent, supervised")
    ...
    print(
        ...
        "the supervisor can only make the system more conservative."
    )
