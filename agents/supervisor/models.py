...
supervisor cannot review five different shapes, so each one is adapted into a common
...
"the supervisor can only ever be more conservative" checkable rather than merely
...
stops a caller reaching for `min()`. The enforcement lives in `agent.py`
...
class Decision(BaseModel):
    """One specialist agent's conclusion, in a form the supervisor can review."""
...
class Review(BaseModel):
    """The supervisor's complete assessment of one decision."""
