...
from agents.improver.models import Finding, Patch, Reviewer, Severity
...
def test_windows_separators_are_folded():
    assert normalise("agents\\improver\\safety.py") == "agents/improver/safety.py"
...
def test_the_eval_suite_is_protected():
    assert is_protected("evals/cases/brain.py")
...
def test_ordinary_source_is_not_protected():
    for path in ("core/agent.py", "agents/brain/codex.py", "console/overlay.py"):
...
def test_a_filename_merely_containing_test_is_not_protected():
    ...
    assert not is_protected("agents/brain/contest.py")
...
def test_the_improver_cannot_patch_itself():
    assert is_self("agents/improver/safety.py")
    assert is_self("agents/improver/pipeline.py")
    assert not may_modify("agents/improver/models.py")


def test_another_agent_is_not_the_improver():
    assert not is_self("agents/brain/codex.py")
...
def test_a_patch_touching_the_improver_is_refused():
    violations = check_patch(patch({"agents/improver/safety.py": "x"}))
