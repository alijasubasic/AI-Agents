...
from agents.improver.models import FileEntry
from agents.improver.safety import is_protected, is_self, normalise
...
def _rank(entry: FileEntry) -> tuple[float, list[str]]:
    ...
    if entry.protected or entry.is_self:
        return 0.0, ["excluded: protected or part of the improver"]
...
def candidates(entries: list[FileEntry], *, limit: int = 3) -> list[FileEntry]:
    ...
    improver that reviews protected files produces findings nobody may act on,
