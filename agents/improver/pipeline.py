...
from agents.improver.patcher import Patcher
from agents.improver.prioritizer import prioritise
from agents.improver.reviewers import ReviewerCrew
from agents.improver.safety import MAX_PATCHES_PER_RUN, MAX_RUN_COST_USD
from agents.improver.verifier import EVAL_COMMAND, verify
...
class ImprovementPipeline:
    ...
    def run(self, index: list[FileEntry]) -> ImprovementRun:
        """Review, patch and verify. Returns everything that happened."""
        result = ImprovementRun(run_date=self.run_date, scanned=index)

        from agents.improver.scanner import candidates
    ...
    def _review(
        self, targets: list[FileEntry], sources: dict[str, str], result: ImprovementRun
    ) -> None:
        for entry in targets:
    ...
    def _patch(
        self, result: ImprovementRun, sources: dict[str, str], *, baseline_output: str
    ) -> None:
    ...
    def _attempt(self, patch: Patch, *, applied: int, baseline: str) -> PatchAttempt:
        ...
        from agents.improver.verifier import check_safety
