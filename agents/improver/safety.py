...
So the improver cannot touch the things that judge it. Not "is told not to";
...
from agents.improver.models import Patch

#: Paths the improver may never modify, whatever it finds there.
#:
...
#: The improver's own package. It may analyse itself and report findings; it
#: may not patch itself. A self-modifying improver is one whose safety rules
#: are editable by the thing they constrain.
SELF_PREFIX = "agents/improver/"
...
def is_protected(path: str) -> bool:
    """Whether a path is one of the checks that judge the improver's work."""
...
def is_self(path: str) -> bool:
    """Whether a path belongs to the improver itself."""
...
def check_patch(patch: Patch, *, applied_so_far: int = 0) -> list[str]:
    ...
    for path in patch.touched:
        if is_self(path):
            violations.append(
                f"{path} is part of the improver itself; changes to it go through a person"
            )
        elif is_protected(path):
            violations.append(
                f"{path} is a test, eval, or CI file; the improver may not modify "
