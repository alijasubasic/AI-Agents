...
def loop(**overrides) -> ImprovementLoop:
    base = {
        "task": build_task(),
    ...
    return ImprovementLoop(**{**base, **overrides})
...
def test_demo_main_runs_without_a_key(capsys, monkeypatch):
    ...
    assert "self-improving demo" in output
...
def test_critiques_describe_patterns_not_individual_cases():
    ...
    from agents.self_improving.scripted import CRITIQUES
