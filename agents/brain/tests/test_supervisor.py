...
The claim this file has to earn: **the brain can only ever be more conservative
...
def test_a_relaxed_reviewer_cannot_approve_what_the_codex_held():
    ...
    brain = BrainAgent(provider=judge(recommend_hold=False), settings=settings())
    review = brain.review(decision(requires_human=True, escalation_reasons=["hostile"]))

    assert review.verdict is Verdict.HOLD_FOR_HUMAN


def test_a_cautious_reviewer_can_hold_what_the_codex_approved():
    brain = BrainAgent(provider=judge(recommend_hold=True, concerns=["tone"]), settings=settings())
    review = brain.review(decision())
...
def test_the_verdict_is_never_looser_than_the_codex_alone():
    ...
    for source, relaxed in itertools.product(cases, [True, False]):
        codex_only = BrainAgent(settings=settings()).review(source).verdict
        with_model = (
            BrainAgent(provider=judge(recommend_hold=relaxed), settings=settings())
            .review(source)
            .verdict
        )
...
def test_an_escalation_survives_every_reviewer_opinion():
    for relaxed in (True, False):
        brain = BrainAgent(provider=judge(recommend_hold=relaxed), settings=settings())
        review = brain.review(decision(requires_human=True))
...
def test_the_model_is_not_consulted_once_the_codex_has_blocked():
    ...
    brain = BrainAgent(provider=provider, settings=settings())

    review = brain.review(decision(recipient_verified=False))
...
def test_the_brain_runs_on_the_codex_alone():
    ...
    review = BrainAgent(settings=settings()).review(decision(requires_human=True))
...
def test_the_outbound_text_reaches_the_reviewer_as_delimited_data():
    ...
    BrainAgent(provider=provider, settings=settings()).review(decision())
...
def test_every_codex_finding_appears_in_the_reasons():
    review = BrainAgent(settings=settings()).review(decision(requires_human=True, trace_ref=None))
...
def test_a_clean_decision_is_approved():
    provider = judge(recommend_hold=False)
    review = BrainAgent(provider=provider, settings=settings()).review(decision())
...
def test_reviewing_many_decisions_preserves_order():
    ...
    reviews = brain.review_all([decision(id=f"d{i}") for i in range(3)])
