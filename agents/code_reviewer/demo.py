"""Runnable demonstration of the improvement agent.
    ...
    python -m agents.code_reviewer.demo
...
code reviewer would look at next.
...
from agents.code_reviewer.models import Patch, PatchStatus, ReviewRun
...
def run(settings: Settings | None = None) -> ReviewRun:
...
def _print_scan(root: Path) -> None:
    ...
    print("=" * 78)
    print(f"  {len(excluded)} excluded as protected or part of the code reviewer itself")
    print("\n  next in line for review:")
    for entry in eligible:
        print(f"    {entry.priority:>4.1f}  {entry.path:<44} {', '.join(entry.priority_reasons)}")


def _print_run(result: ReviewRun) -> None:
    print(f"\n{'=' * 78}")
...
def _print_safety() -> None:
    ...
    probes: list[tuple[str, Patch]] = [
        ...
        (
            "a patch that would edit the code reviewer itself",
            Patch(
                finding=DISCOUNT_BUG,
                branch="improve/probe-b",
                changes={"agents/code_reviewer/safety.py": "MAX_PATCHES_PER_RUN = 999\n"},
            ),
        ),
        (
