...
def render_entry(run: ReviewRun) -> str:
...
def _reviewer_counts(run: ReviewRun) -> list[tuple[str, int]]:
...
def render_log(runs: list[ReviewRun]) -> str:
...
def summarise(run: ReviewRun) -> str:
...
def worst_unfixed(run: ReviewRun) -> list[str]:
