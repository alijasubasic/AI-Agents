...
So every guardrail in [`safety.py`](safety.py) points at the code reviewer itself:
...
**Regression tests are written, not added.** The code reviewer cannot write into
...
- **One finding, one file.** That keeps patches reviewable and rules out a whole
  ...
  every caller, so the code reviewer reports it and stops.
