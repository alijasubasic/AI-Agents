"""`make improve`.
    ...
    python -m agents.code_reviewer             # dry run: scan, review, report
    python -m agents.code_reviewer --apply     # also write patches, on branches
...
from agents.code_reviewer.models import Reviewer, ReviewRun
from agents.code_reviewer.pipeline import ReviewPipeline
from agents.code_reviewer.prioritizer import prioritise
from agents.code_reviewer.reporter import LOG_PATH, render_entry, summarise
from agents.code_reviewer.reviewers import ReviewerCrew
from agents.code_reviewer.safety import MAX_PATCHES_PER_RUN
...
def dry_run(root: Path, *, settings: Settings, limit: int) -> ReviewRun:
    """Scan and review. Draft nothing, write nothing."""
    index = scan(root)
    result = ReviewRun(run_date=date.today(), scanned=index)
...
def apply_run(root: Path, *, settings: Settings, limit: int, max_patches: int) -> ReviewRun:
    """The full pipeline against a real checkout."""
    from agents.code_reviewer.workspace import GitWorkspace
...
def _print(result: ReviewRun, *, applied: bool) -> None:
    print(f"\n{'=' * 78}")
