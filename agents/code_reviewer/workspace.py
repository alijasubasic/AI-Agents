...
class Workspace(Protocol):
    """Everything the code reviewer needs to do to a checkout."""
...
class GitWorkspace:
    ...
    def __init__(self, root: Path | str = ".", *, base_branch: str = "main") -> None:
        ...
        if status.output.strip():
            raise RuntimeError(
                "the working tree has uncommitted changes. The code reviewer discards "
    ...
    def create_branch(self, name: str) -> None:
        if not name.startswith("improve/"):
            raise ValueError(
                f"refusing to create {name!r}: the code reviewer only creates branches "
